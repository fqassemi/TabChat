...
(() => {
  ...
  async function injectChatWidget() {
    ...
    style.textContent = `
      ...
      #chat-widget * { box-sizing: border-box; font-family: system-ui, sans-serif; }
      #chat-widget input { outline: none; }
      ...
      #cw-header button:hover { background: rgba(255,255,255,0.18); opacity: 1; }
      ...
      .cw-empty .cw-empty-icon { font-size: 26px; }
      ...
      .cw-bubble.rtl { direction: rtl; text-align: right; }
      .cw-bubble.ltr { direction: ltr; text-align: left; }
    ...
    function showEmptyState() {
      messages.innerHTML = `
        <div class="cw-empty">
          ...
          <div>Ask me anything about this page.<br>I'll answer using its saved content.</div>
        </div>
      `;
    }
    showEmptyState();
    ...
    function addMessage(text: string, fromUser = true) {
      if (messages.querySelector(".cw-empty")) messages.innerHTML = "";

      const msg = document.createElement("div");
      const rtl = isRTL(text);
      msg.textContent = text;
      msg.className = `cw-bubble ${fromUser ? "user" : "bot"} ${rtl ? "rtl" : "ltr"}`;
    ...
    sendBtn.innerHTML = `
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 11.5L21 3L13 21L11 13L3 11.5Z" fill="currentColor"/>
      </svg>
    `;
    ...
    // ===== HEADER ACTIONS =====
    let minimized = false;
    const minimizeBtn = header.querySelector("#cw-minimize") as HTMLButtonElement;
    const closeBtn = header.querySelector("#cw-close") as HTMLButtonElement;
    ...
    closeBtn.addEventListener("click", () => {
      chatDiv.style.transition = "opacity 0.15s ease, transform 0.15s ease";
      chatDiv.style.opacity = "0";
      chatDiv.style.transform = "translateY(10px) scale(0.97)";
      setTimeout(() => {
        chatDiv.remove();
