...
export class MoorchehClient {
    constructor(apiKey) {
        if (!apiKey)
            throw new Error("Moorcheh API key required");
        this.apiKey = apiKey;
    }
    headers() {
        return {
    ...
    async createNamespace(namespaceName, type = "text", vector_dimension) {
        const body = { namespace_name: namespaceName, type };
        if (type === "vector")
            body.vector_dimension = vector_dimension || 1536;
    ...
    async uploadTextDocuments(namespaceName, documents) {
    ...
    async search(namespaceNames, query, options) {
        ...
        if (typeof query === "string") {
            body.query = query;
            if (body.kiosk_mode && typeof options?.threshold === "number")
...
//# sourceMappingURL=moorchehClient.js.map